        ...
        'func': () => {
          insertCodeFromFile({
            ...
            'func': () => {
              ...
              spec.get().load = (ele) => {
                if(localStorage.hasOwnProperty(ele.id))
                  ele.value = localStorage[ele.id];
              };
              if(spec.hasOwnProperty('store')) {
                spec.store.forEach((n) => {
                  spec.get().load(spec.get().store.ele[n])
                })
              }
              ...
              spec.get().model = initModel(spec.get(), () => {
                colors.forEach((n) => {
                  spec.get().save(spec.get().color.ele[n])
                });
                if(spec.hasOwnProperty('store')) {
                  spec.store.forEach((n) => {
                    spec.get().save(spec.get().store.ele[n])
                  })
                }
              });
